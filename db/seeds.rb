...
def seed
  reset_db
  create_collections(100)
  create_items(2..8)
  create_selected_tags
end
...
def create_collections(quantity)
  quantity.times do
    ...
    puts "Collection with title #{collection.title} just created"
  end
end
...
def create_selected_tags
  Collection.all.each do |collection|
    rand(1..4).times do
      ...
      selected_tag = SelectedTag.create!(tag: tag, collection: collection)
      puts "SelectedTag created for collection #{selected_tag.collection.title} with tag #{tag.name}"
    end
  end
end
